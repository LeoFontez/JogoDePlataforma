	...
	if fruits < 0:
		...
		for h in range(divY):
			for w in range(divX):
				...
				sprite.global_position = $BreakPosition.global_position
...
func createFruits():
	var fruitNumber = round(rand_range(0, fruit_instance.size() - 1))
	var fruit = fruit_instance[fruitNumber].instance()
